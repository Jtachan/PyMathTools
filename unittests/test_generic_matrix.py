...
@pytest.mark.parametrize(
    "mat_a",
    (
        np.array([[randint(-5, 5), randint(-5, 5)], [randint(-5, 5), randint(-5, 5)]]),
        np.array([[randint(-5, 5), randint(-5, 5)], [randint(-5, 5), randint(-5, 5)]]),
    ),
)
@pytest.mark.parametrize(
...
def test_assert_math_operands(mat_a, mat_b):
    ...
    assert gen_mat_a + gen_mat_b == mat_a + mat_b, "Sum operand failed"
    assert gen_mat_a - gen_mat_b == mat_a - mat_b, "Subtract operand failed"

    assert gen_mat_a * 1.5 == mat_a * 1.5, "Number multiplication failed"
